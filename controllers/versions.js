...
exports.createVersion = asyncHandler(async (req, res, next) => {
  ...
  if (req.body.version > 1) {
    try {
      ...
      const elements = await RevitElement.find({
        project: projectId,
        version: oldVersion._id
      });
      ...
      next(
        new ErrorResponse(` Fail because no element in the precedent version`)
      );
    }
  }

  version = await Version.findOneAndUpdate(
    { project: projectId },
    { $push: { versions: req.body } },
    {
      new: true,
      runValidators: true
    }
...
exports.deleteProjectVersion = asyncHandler(async (req, res, next) => {
  ...
  if (!version)
    return next(
      new ErrorResponse(
        `Version ${req.body.version} of project ${req.params.projectId} is not found`
      )
    );
  ...
  const nbrDeletedElement = await RevitElement.deleteMany({
    version: deletedVersionId
  }).countDocuments();

  //Delete version
  version = await Version.findOneAndUpdate(
    { project: req.params.projectId },
    { $pull: { versions: { version: req.body.version } } },
    {
      new: true,
      runValidators: true
    }
