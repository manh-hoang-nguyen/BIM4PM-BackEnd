...
exports.getUserProjects = asyncHandler(async (req, res, next) => {
  const projects = await Project.find({ owner: req.user.id }).select(
    '-members'
  );
  res.status(200).json({
    success: true,
...
exports.createProject = asyncHandler(async (req, res, next) => {
  ...
  if (
    ...
    return next(
      new ErrorResponse(
        `The user with Id ${req.user.id} has already created a project`,
        403
      )
    );
  }
...
exports.addMember = asyncHandler(async (req, res, next) => {
  ...
  if (!memberToAdd) {
    return next(
      new ErrorResponse(`User ${req.body.email} is not registerd yet`, 400)
    );
  }
  ...
  if (project.owner.toString() !== req.user.id && req.user.status !== 'admin') {
    ...
    if (!userRole || userRole !== 'administrator')
      return next(
        new ErrorResponse(
          `User ${req.user.email} is not authorized to update this project`,
          401
        )
      );
    ...
    if (checkUser)
      return next(
        new ErrorResponse(
          `User ${memberToAdd.email} is in this project already`,
          401
        )
      );
  }
...
exports.removeMember = asyncHandler(async (req, res, next) => {
  ...
  if (project.owner.toString() !== req.user.id && req.user.status !== 'admin') {
    ...
    if (!checkUser)
      return next(
        new ErrorResponse(
          `User ${memberToRemove.email} is not in this project`,
          401
        )
      );
  }
